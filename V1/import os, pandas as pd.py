...
class Generator(nn.Module):
    def __init__(self, in_ch, out_ch, n_blocks=6):  # reduced to 6 for speed
        super().__init__()
        layers = [
        ...
        for _ in range(2):  # upsample
            layers += [nn.ConvTranspose2d(dim, dim//2, 3, stride=2, padding=1, output_padding=1), nn.InstanceNorm2d(dim//2), nn.ReLU(True)]
            dim //= 2
        layers += [nn.ReflectionPad2d(3), nn.Conv2d(dim, out_ch, 7), nn.Tanh()]
...
class Discriminator(nn.Module):
    def __init__(self, in_ch):
        super().__init__()
        layers = [nn.Conv2d(in_ch, 64, 4, stride=2, padding=1), nn.LeakyReLU(0.2, True)]
        dim = 64
        for mult in [2, 4, 8]:
            layers += [nn.Conv2d(dim, dim*mult, 4, stride=2, padding=1), nn.InstanceNorm2d(dim*mult), nn.LeakyReLU(0.2, True)]
            dim *= mult
...
class VGGFeatureExtractor(nn.Module):
    def __init__(self, layers=('3', '8', '15')):
        ...
        self.vgg_parts = nn.ModuleList([vgg[:int(l)+1].eval() for l in layers])
        for p in self.vgg_parts.parameters(): p.requires_grad = False

    def forward(self, x):
        features = []
        for part in self.vgg_parts:
            x = part(x)
            features.append(x)
        return features
...
def train(args):
    ...
    vgg_extractor = VGGFeatureExtractor().to(args.device)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for i, batch in enumerate(loader):
            ...
            with autocast():
                ...
                # Perceptual
                feats_realA = vgg_extractor(realA)
                feats_fakeA = vgg_extractor(fakeA)
                loss_percA = sum(criterion_cycle(fa, fr) for fa, fr in zip(feats_fakeA, feats_realA))
                feats_realB = vgg_extractor(realB)
                feats_fakeB = vgg_extractor(fakeB)
                loss_percB = sum(criterion_cycle(fb, fr) for fb, fr in zip(feats_fakeB, feats_realB))
                # Total
                loss_G = (
                    loss_GAN_A2B + loss_GAN_B2A
                    + args.lambda_cycle * loss_cycle
                    + args.lambda_id * loss_id
                    + perceptual_weight * (loss_percA + loss_percB)
                )
            ...
            with autocast():
                ...
                loss_D_B = 0.5 * (criterion_GAN(pred_realB, torch.ones_like(pred_realB)) + criterion_GAN(pred_fakeB, torch.zeros_like(pred_fakeB)))
                pred_realA = D_A(realA); pred_fakeA = D_A(fakeA.detach())
                loss_D_A = 0.5 * (criterion_GAN(pred_realA, torch.ones_like(pred_realA)) + criterion_GAN(pred_fakeA, torch.zeros_like(pred_fakeA)))
            ...
            if (i + 1) % args.log_interval == 0:
                ...
                iters = args.log_interval
                print(f"Epoch[{epoch}/{args.epochs}] Step[{i+1}/{len(loader)}] "
                      f"G: {loss_G.item():.4f}, D: {loss_D.item():.4f}  "
                      f"({iters/elapsed:.2f} iters/sec, {elapsed:.1f}s/{iters} iters)")
                start_time = time.time()

        # Save samples
        with torch.no_grad():
            sampleA = realA[:4]; genB = G_AB(sampleA)
            save_image(torch.cat((sampleA, genB), 0), os.path.join(args.output, f"epoch{epoch}_sample.jpg"), nrow=4, normalize=True)
        # Save checkpoint
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=30, help="total epochs to train")
    ...
    parser.add_argument("--save_interval", type=int, default=5, help="epochs between checkpoints")
